...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET' && req.query.action === 'callback') {
    ...
    try {
      ...
      if (isAdmin) {
        return res.redirect(`${redirectTo}/newsletter-admin.html`);
      } else {
  ...
  if (req.method === 'POST' && req.query.action === 'refresh') {
    try {
      ...
      // For now, we'll need to reconstruct or fetch user data
      // In production, you might want to store user data in a session store
      const userInfo = {
